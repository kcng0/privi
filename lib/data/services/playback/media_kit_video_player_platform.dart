...
/// handle that profile, size, or frame rate (e.g. 2560x1440 @ 120fps).
...
class MediaKitVideoPlayerPlatform extends VideoPlayerPlatform {
  ...
  Future<int?> createWithOptions(VideoCreationOptions options) async {
    ...
    if (slot.error != null && isDecoderCapabilityError(slot.error!)) {
      await slot.dispose();
  ...
  void tryComplete() {
    ...
    completer.complete(
      VideoEvent(
        ...
        rotationCorrection: player.state.videoParams.rotate ?? 0,
      ),
    );
  }
